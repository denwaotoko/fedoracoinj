 ...
 * the lightweight SPV mode, a {@link com.google.fedoracoin.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.google.fedoracoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        if ((storedPrev.getHeight() + 1) % params.getInterval() != 0 && (storedPrev.getHeight()+1) < 51000) { //(storedPrev.getHeight()+1) < 51000) <- this condition could be a very bad choice, idk
        ...
        if (storedPrev.getHeight()+1 < 51000){
        ...
        if (newDifficulty.compareTo(receivedDifficulty) != 0)
            throw new VerificationException("Network provided difficulty bits do not match what was calculated (below 51000): " +
    ...
    private void checkDifficultyTransitions_V2(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        long	PastSecondsMax	= TimeDaySeconds * 1; // 1 day
...
bnNew /= PastRateTargetSeconds;
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in fedoracoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
