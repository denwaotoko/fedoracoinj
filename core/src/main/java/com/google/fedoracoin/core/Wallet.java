...
import com.google.fedoracoin.core.TransactionConfidence.ConfidenceType;
import com.google.fedoracoin.crypto.KeyCrypter;
import com.google.fedoracoin.crypto.KeyCrypterException;
import com.google.fedoracoin.crypto.KeyCrypterScrypt;
import com.google.fedoracoin.script.Script;
import com.google.fedoracoin.script.ScriptBuilder;
import com.google.fedoracoin.script.ScriptChunk;
import com.google.fedoracoin.store.UnreadableWalletException;
import com.google.fedoracoin.store.WalletProtobufSerializer;
import com.google.fedoracoin.utils.ListenerRegistration;
import com.google.fedoracoin.utils.Threading;
import com.google.fedoracoin.wallet.*;
import com.google.fedoracoin.wallet.WalletTransaction.Pool;
...
import static com.google.fedoracoin.core.Utils.bitcoinValueToFriendlyString;
import static com.google.fedoracoin.core.Utils.bitcoinValueToPlainString;
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.fedoracoin.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.google.fedoracoin.core.Wallet#getBalance()} - the fees required
         ...
         *
         * <p>Once {@link Wallet#completeTx(com.google.fedoracoin.core.Wallet.SendRequest)} is called, this is set to the
        ...
        /**
         * If not null, the {@link com.google.fedoracoin.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // We need to know if we need to add an additional fee because one of our values are smaller than 1 TIPS
            boolean needAtLeastReferenceFee = false;
            int txOutDustFeeCount = 0;
            if (req.ensureMinRequiredFee && !req.emptyWallet) { // min fee checking is handled later for emptyWallet
                for (TransactionOutput output : req.tx.getOutputs())
                    if (output.getValue().compareTo(Utils.COIN) < 0) { //TXOut lower than 1 TIPS have a 2 TIPS fee!
...
//                            throw new IllegalArgumentException("Tried to send dust with ensureMinRequiredFee set - no way to complete this");
                        ...
                        txOutDustFeeCount++; //TIPS: Each TXOut < 1 TIPS needs +1 TIPS fee!
                    }
            }
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.fedoracoin.wallet.WalletFiles.Listener)}
     ...
     * of {@link com.google.fedoracoin.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.google.fedoracoin.crypto.KeyCrypter,
     ...
     * {@link com.google.fedoracoin.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all fedoracoin controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.google.fedoracoin.crypto.KeyCrypter,
     ...
     * you can use {@link com.google.fedoracoin.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                //TIPS: Add 1 TIPS fee per txOut < 1 TIPS
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    if (req.ensureMinRequiredFee && Transaction.MIN_NONDUST_OUTPUT.compareTo(change) >= 0) {
                        ...
                        additionalValueForNextCategory = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.add(BigInteger.ONE);
                                                         //TIPS: We don't have a min value, but we add more fees for tx < 1
                    } else {
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.google.fedoracoin.core.TransactionBroadcaster broadcaster) {
        lock.lock();
        try {
