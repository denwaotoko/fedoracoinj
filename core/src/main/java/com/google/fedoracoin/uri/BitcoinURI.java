 ...
 *
 * <ul>
 * <li>{@code fedoracoin:<address>}</li>
 * <li>{@code fedoracoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
...
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "fedoracoin";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  fedoracoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form fedoracoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the fedoracoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : fedoracoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("fedoracoin://")) {
            schemeSpecificPart = input.substring("fedoracoin://".length());
        } else if (input.startsWith("fedoracoin:")) {
            schemeSpecificPart = input.substring("fedoracoin:".length());
        } else {
